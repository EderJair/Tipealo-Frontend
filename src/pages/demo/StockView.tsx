    ...
    <div className="p-6 space-y-4" style={{ fontFamily: 'Lora, serif' }}>
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="metric-card">
          ...
          <p className="mt-2.5 text-[2.65rem] font-semibold text-white leading-none">{products.length}</p>
          <p className="mt-2 text-[1rem] text-white/60">artículos en catálogo</p>
        </div>
        <div className="metric-card">
          <p className="text-[11px] font-semibold uppercase tracking-[0.26em] text-white/60">Valor inventario</p>
          <p className="mt-2.5 text-[2.65rem] font-semibold text-white leading-none">S/ {totalValue.toLocaleString()}</p>
          <p className="mt-2 text-[1rem] text-white/60">en stock hoy</p>
        </div>
        <div className="metric-card">
          <p className="text-[11px] font-semibold uppercase tracking-[0.26em] text-white/60">Stock bajo</p>
          <p className="mt-2.5 text-[2.65rem] font-semibold text-white leading-none">{bajo}</p>
        ...
        <div className="metric-card">
          ...
          <p className="mt-2.5 text-[2.65rem] font-semibold text-white leading-none">{critico}</p>
          <p className="mt-2 text-[1rem] text-white/60">reposición urgente</p>
          <div className="mt-3 flex items-center gap-1 text-[13px] font-semibold text-red-400">revisar ahora</div>
        </div>
      </div>

      {/* Alert */}
      {critico > 0 && (
        <div className="flex items-center gap-3 bg-red-50 border border-red-100 px-5 py-3">
          <div className="w-1.5 h-1.5 bg-red-500 rounded-full shrink-0" />
          <p className="text-[12px] font-medium text-red-700">
            {products.filter(p => p.status === 'crítico').map(p => p.name).join(' · ')} — sin unidades disponibles
          </p>
        </div>
      )}
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-black/5">
              {['Producto', 'Categoría', 'Unidad', 'Stock', 'Precio unit.', 'Valor total', 'Estado'].map(col => (
          ...
          <tbody>
            {products.map((p, i) => (
              <tr key={p.name} className={`hover:bg-gray-50/50 transition-colors ${i < products.length - 1 ? 'border-b border-black/4' : ''}`}>
                <td className="px-5 py-3 text-[12px] font-semibold text-black">{p.name}</td>
                <td className="px-5 py-3 text-[11px] text-gray-500">{p.cat}</td>
                <td className="px-5 py-3 text-[11px] text-gray-500">{p.unit}</td>
                <td className="px-5 py-3 text-[14px] font-bold text-black">{p.stock}</td>
                <td className="px-5 py-3 text-[12px] text-gray-700">S/ {p.price.toFixed(2)}</td>
                <td className="px-5 py-3 text-[12px] font-semibold text-black">S/ {(p.stock * p.price).toLocaleString()}</td>
